 ...
 * 自定义分区器 + 减少 shuffle次数
 */
object TheMostPopularTeacherCustomPartitionerTreeSetTopN2 {
  def main(args: Array[String]): Unit = {
    ...
    val subjectAndTeacher: RDD[((String, String), Int)] = rdd.map(line => {
      ...
      ((subject, teacher), 1)
    }).filter(t => t._1._1 != null || t._1._2 != null)
    ...
    //按照学科进行分区
    val result: RDD[((String, String), Int)] = subjectAndTeacher.reduceByKey(myPartioner, _ + _).mapPartitions(it => {
      /**
